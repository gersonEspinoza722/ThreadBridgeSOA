...
*/
	...
	if(direction == TO_EAST){
		...
		while(west_count > 0 || east_count >= max_load){ //Si no hay trafico en la otra direccion y espacio, espere senial
		...
		while(east_count > 0 || west_count >= max_load){ //Si no hay trafico en la otra direccion y espacio, espere senial
...
int main(int argc, char *argv[]){
	...
	pthread_t cars[amount_of_traffic]; //Todo los hilos/carros
	...
	//Crear todos los hilos/carros y asignarles un indice
	for(i = 0; i < amount_of_traffic; i++){
